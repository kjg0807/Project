									...
									<c:otherwise>
										<td>
											<button class="btn btn-outline-dark"
												onclick="location.href='./detail?num=${pageScope.dto.num}';"
												type="submit" style="width: auto;">클릭</button>
										</td>
									</c:otherwise>
								</c:choose>
							</tr>
						...
						<ul class="pagination">
							<c:if test="${reviewsPager.pre}">
								<li class="page-item"><a class="page-link"
										href="./list?page=${reviewsPager.startNum-1}&kind=${reviewsPager.kind}&search=${reviewsPager.search}"
							...
							<c:forEach begin="${reviewsPager.startNum}" end="${reviewsPager.lastNum}" var="i">
								<li class="page-item"><a class="page-link"
										href="./list?page=${i}&kind=${reviewsPager.kind}&search=${reviewsPager.search}">${i}</a>
								</li>
							</c:forEach>
							<li class="page-item ${reviewsPager.next?'':'disabled'}"><a class="page-link"
									href="./list?page=${reviewsPager.lastNum+1}&kind=${reviewsPager.kind}&search=${reviewsPager.search}"
